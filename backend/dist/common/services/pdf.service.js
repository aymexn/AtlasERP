...
let PdfService = class PdfService {
    ...
    assertColumnWidths(widths, usableWidth) {
    ...
    str(val, fallback = 'N/A') {
        if (val === null || val === undefined || val === '')
            return fallback;
        return String(val);
    }
    async generatePayslipPdf(run, res) {
        if (!run)
            throw new common_1.InternalServerErrorException('No payroll run data provided');
        ...
        try {
            ...
            throw new common_1.InternalServerErrorException('Error generating payslip PDF');
        }
    }
    async generateInvoicePdf(invoice, res) {
        if (!invoice)
            throw new common_1.InternalServerErrorException('No invoice data provided');
        ...
        try {
            ...
            throw new common_1.InternalServerErrorException('Error generating invoice PDF: ' + error.message);
        }
    }
    async generateSalesOrderPdf(order, res) {
        return this.generateDocument(order, res, 'BON DE COMMANDE CLIENT');
    }
    async generatePurchaseOrderPdf(order, res) {
        return this.generateDocument(order, res, 'BON DE COMMANDE FOURNISSEUR');
    }
    async generateWorkOrderPdf(order, res) {
        try {
            ...
            throw new common_1.InternalServerErrorException('Error generating work order PDF: ' + error.message);
        }
    }
    async generateInventoryPdf(company, products, res) {
        try {
            const doc = new pdfkit_1.default({ margin: 50, size: 'A4' });
            doc.pipe(res);
            ...
            throw new common_1.InternalServerErrorException('Error generating inventory PDF: ' + error.message);
        }
    }
    async generateSupplierPdf(company, supplier, res) {
        try {
            const doc = new pdfkit_1.default({ margin: 50, size: 'A4' });
            doc.pipe(res);
            ...
            throw new common_1.InternalServerErrorException('Error generating supplier PDF: ' + error.message);
        }
    }
    async generateExpensesPdf(company, expenses, res) {
        try {
            const doc = new pdfkit_1.default({ margin: 50, size: 'A4' });
            doc.pipe(res);
            ...
            console.error('Stack:', error.stack);
            throw new common_1.InternalServerErrorException('Error generating expenses PDF: ' + error.message);
        }
    }
    drawOfficialHeader(doc, company, title) {
        const headerTop = 50;
        ...
        doc.fillColor('#666666').fontSize(8).font('Helvetica')
            ...
            .text(`NIF: ${this.str(company?.nif)} | RC: ${this.str(company?.rc)} | AI: ${this.str(company?.ai)}`, 50, headerTop + 97);
    ...
    drawFooter(doc, company) {
        ...
        doc.fontSize(7).fillColor('#aaaaaa').font('Helvetica')
            .text(`AtlasERP — Document de Gestion Interne — ${companyName}`, 50, footerTop + 8, { align: 'center', width: 500 });
    }
    async generateDocument(docData, res, title) {
        try {
            const doc = new pdfkit_1.default({ margin: 50, size: 'A4' });
            doc.pipe(res);
            ...
            throw new common_1.InternalServerErrorException('Error generating PDF document: ' + error.message);
        }
    }
    generateLine(doc, y, color = '#dddddd') {
